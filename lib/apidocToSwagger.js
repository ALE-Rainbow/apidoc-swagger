...
function toSwagger(apidocJson, projectJson, swaggerInit) {
  ...
  Object.values(tags).forEach( t => {
    if( !swagger.tags.some( st => st.name === t) ){
      console.error(`\x1b[31merror:\x1b[0m Operation tags "${t}" must be defined in global tags.`)
    }
  });

  // Sort tags
  swagger.tags = swagger.tags.sort((a, b) => ('' + a.name).localeCompare(b.name));

  // Check Tag Groups
  swagger.tags.forEach( t => {
    if( !swagger["x-tagGroups"].some( tg => tg.tags.includes(t))) {
      console.error(`\x1b[33mwarn:\x1b[0m Operation tags "${t.name}" does not belong to a group in "x-tagGroups".`)
    }
  });
...
function createPostPushPutOutput(verbs, components, pathKeys) {
  ...
  let contentType =
    verbs?.header?.fields["Header"]?.find((value) =>
      ["content-type"].includes(value.field)
  ...
  if (verbs.parameter && verbs.parameter.fields["Body Parameters"]) {
    ...
    requestBody.content[contentType] = {
      schema: {
        $ref:
          "#/components/schemas/" + verbDefinitionResult.topLevelParametersRef,
      },
    };
  }
  ...
  if (verbDefinitionResult.topLevelSuccessRef) {
    ...
    verbs?.success?.headers.some((value) => {
      if (["Content-Type"].includes(value.field)) {
  ...
  if( !pathItemObject[verbs.type].description ) {
    console.warn( `\x1b[33mwarn:\x1b[0m Operation "@apiDescription" must be present and non-empty string, for @apiName ${pathItemObject[verbs.type].operationId}`)
  }
            ...
            if (param.type.toLowerCase() === "string") {
              if (param.size) {
                ...
                if (!match && param.size.match(/\d+/)) {
                  item.schema.minLength = item.schema.maxLength = parseInt(param.size, 10);
                }
              }
            }

            if (param.type.toLowerCase() === "integer" || param.type.toLowerCase() === "number") {                
              if (param.size) {
                var sizeNumberRegex = /^(-?\d*)-(\d*)$/g; // ex: "4-67"
