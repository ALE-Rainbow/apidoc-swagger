	...
	if (defaultValue) {
		...
		if (defaultValue && defaultValue.startsWith('/')) {
			...
			if (type === "number") {
				...
				console.log(`[extractDefaultValues] Handling string[] for defaultValue ${defaultValue}`);
				try {
					prop.default = JSON.parse(defaultValue);
				} catch(e) {
					console.warn(`[extractDefaultValues] Failed to JSON parse defaultValue ${defaultValue}, set it without parsing`);
