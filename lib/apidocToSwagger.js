...
function apidoc2SwaggerMarkdownLinks(value) {
	try {
		return value ? value.replace(/(\[.*\])\((\/.*\/)?(index\.html)?(#api-(.*)-)(.*)\)/ig, (m, g1, g2, g3, g4, g5, g6) => `${g1}(${g2 ? `/doc/rest/api${g2}/redoc-index.html` : ''}#operation/${g6.charAt(0).toLowerCase() + g6.slice(1)})`) : value;
	} catch (err) {
		return value;
	}
}
...
function addInfo(projectJson) {
	...
	if (projectJson.header) {
		...
		info["description"] = apidoc2SwaggerMarkdownLinks(projectJson.description);
	}
	return info;
}
...
function extractAllowedValues(allowedValues, type) {
	...
	if (allowedValues) {
		...
		if (allowedValues[0] && allowedValues[0].startsWith('/')) {
			...
			if (type === 'number' && allowedValues[0].includes("..")) {
				throw new Error("Number range must be set using {number{100-999}} and not " + allowedValues[0]);
				console.log(allowedValues[0])
			}
			prop.enum = type === "number" ? allowedValues.map(function (value) {
...
function extractDefaultValues(defaultValue, type) {
	...
	if (defaultValue) {
		...
		if (defaultValue && defaultValue.startsWith('/')) {
			prop.pattern = type === "number" ? parseInt(defaultValue, 10) : defaultValue;
		} else {
			if (type === "number") {
				prop.default = parseInt(defaultValue, 10);
			} else if (type === "boolean") {
				prop.default = defaultValue.toLowerCase() === "true";
			} else if ((type === "string[]" || type === "object[]") && defaultValue !== "undefined") {
				console.log(`[extractDefaultValues] Handling type ${type} for defaultValue ${defaultValue}`);
				try {
					prop.default = JSON.parse(defaultValue);
				} catch(e) {
					console.warn(`[extractDefaultValues] Failed to JSON parse defaultValue ${defaultValue} for type ${type}, set it without parsing`);
...
function createPostPushPutOutput(verbs, definitions, pathKeys) {
	...
	pathItemObject[verbs.type] = {
		...
		description: removeTagsWithHtml2markdown(apidoc2SwaggerMarkdownLinks(verbs.description)),
	...
	if (verbs.deprecated) {
		...
		pathItemObject[verbs.type].description = apidoc2SwaggerMarkdownLinks(verbs.deprecated.content);
	}
	...
	if (verbDefinitionResult.topLevelErrorArray && verbDefinitionResult.topLevelErrorArray.length) {
		...
		verbDefinitionResult.topLevelErrorArray.forEach(function (item) {
			pathItemObject[verbs.type].responses[item.code] = {
				description: apidoc2SwaggerMarkdownLinks(item.description)
			};
		});
					...
					for (var itemKey in errorItem) {
						if (errorItem.hasOwnProperty(itemKey)) {
							result.topLevelErrorArray.push({
								...
								description: apidoc2SwaggerMarkdownLinks(errorItem[itemKey].description)
							});
						}
					}
				}
	...
	for (var i = 0; i < fieldArray.length; i++) {
		...
		if (nestedName.propertyName) {
			...
			if (parameter.type === "Object") {
				...
				prop.type = proptype;
				prop.description = removeTagsWithHtml2markdown(apidoc2SwaggerMarkdownLinks(parameter.description));
			}

			prop = Object.assign(prop, extractAllowedValues(parameter.allowedValues, prop.type), extractDefaultValues(parameter.defaultValue, prop.type));
			...
			if (typeIndex !== -1 && typeIndex === (type.length - 2)) {
				...
				if (_type === 'object') {
					var localDefinitionName = camelCase(defaultObjectName + " " + nestedName.propertyName);
					prop.items = {
			...
			if (nestedName.objectName && !definitions[camelCase(defaultObjectName + " " + nestedName.objectName)]) {
				...
				if (definitions[camelCase(defaultObjectName + " " + nestedName.objectName)]) {
					objectName = camelCase(defaultObjectName + " " + nestedName.objectName);
				}
		...
		for (var i = 0; i < verbs.permission.length; i++) {
			...
			if (!permissions[permission.name]) {
				permissions[permission.name] = {
					...
					description: removeTagsWithHtml2markdown(apidoc2SwaggerMarkdownLinks(permission.description))
				}
			}
		}
	}
							...
							case 'bearerauthorization':
							{
								...
								if (!security[bearerKey]) {
									security[bearerKey] = {
										...
										in: 'header',
										type: param.type,
										description: removeTagsWithHtml2markdown(apidoc2SwaggerMarkdownLinks(param.description))
									}
								}
							}
								break;
			...
			if (verbs.parameter.fields.hasOwnProperty(key)) {
				for (var i = 0; i < verbs.parameter.fields[key].length; i++) {
					...
					if (param.group && param.group.toLowerCase() === 'urlqueryparameters') {
						...
						item = Object.assign(item, extractAllowedValues(param.allowedValues, param.type.toLowerCase()), extractDefaultValues(param.defaultValue, param.type.toLowerCase()));
				...
				for (var i = 0; i < verbs.parameter.fields[key].length; i++) {
					...
					if (param.group && param.group.toLowerCase() === 'headerparameters') {
						...
						var field = Object.assign({
							...
							extractAllowedValues(param.allowedValues, param.type.toLowerCase()),
