...
function toSwagger(apidocJson, projectJson, swaggerInit) {
  ...
  swagger.tags.forEach((t) => {
    if (
      !swagger["x-tagGroups"].some((tg) =>
        tg.tags.some((tgt) => tgt === t.name)
      )
    ) {
      console.error(
  ...
  if (defaultValue) {
    ...
    if (defaultValue && defaultValue.startsWith("/")) {
      ...
      if (type === "number") {
        ...
        console.debug(`[extractDefaultValues] Handling type ${type} for defaultValue ${defaultValue}`);
        try {
...
function createPostPushPutOutput(verbs, components, pathKeys) {
  ...
  let contentTypeHeader =
    ...
    verbs.header.fields["Header"].find((value) =>
      ["content-type"].includes(value.field.toLowerCase())
    );

  let contentTypes =
    contentTypeHeader && contentTypeHeader.allowedValues
      ? contentTypeHeader.allowedValues
      : contentTypeHeader && contentTypeHeader.description
...
function createFieldArrayComponents(
  ...
  for (var i = 0; i < fieldArray.length; i++) {
    ...
    var nestedName = createNestedName(parameter);
    ...
    var isMap = false;
    if (parameter.type.startsWith("Map(")) {
      parameter.type = parameter.type.replace(/Map\((.*)\)/, "$1");
      isMap = true;
    }
    ...
    if (nestedName.propertyName) {
      ...
      var prop = isMap ? { isMap } : {};
      if (parameter.type === "Object") {
        prop = { ...prop, type: "object", properties: {} };
      } else {
        prop.type = proptype;
      }
      prop = Object.assign(
        prop,
        { description: removeTagsWithHtml2markdown(parameter.description) },
      ...
      if (
        ...
        if (!parameter.optional && !nestedName.isMap) {
  ...
  // Map/Dictionnary - Additional Properties conversion
  var parseObject = (parentObject) => {
    if (!parentObject || !parentObject.properties) return;
    for (const [key, value] of Object.entries(parentObject.properties)) {
      if (value.type === "array") {
        if (value.items.$ref) {
          parseObject(
            components.schemas[
              value.items.$ref.replace("#/components/schemas/", "")
            ]
          );
        }
      } else {
      ...
      if (value.isMap) {
        delete value.isMap;
        parentObject.additionalProperties = { ...value };
        delete parentObject.properties[key];
        if (Object.keys(parentObject.properties).length === 0) {
...
function createNestedName(parameter) {
  var propertyName = parameter.field;
  var objectName;
  var propertyNames = parameter.field.split(".");
  ...
  return {
    isMap: parameter.type.startsWith("Map("),
  ...
  if (verbDefinitionResult.topLevelSuccessRef) {
    if (
      ...
      let contentTypeHeader =
        ...
        verbs.success &&
        verbs.success.headers &&
        verbs.success.headers.find((value) =>
          ["content-type"].includes(value.field.toLowerCase())
        );
      let contentTypes =
        contentTypeHeader && contentTypeHeader.allowedValues
