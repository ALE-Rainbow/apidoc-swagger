...
function apidoc2SwaggerMarkdownLinks(value) {
	try {
		return value ? value.replace(/(\[.*\])\((\/.*\/)?(index\.html)?(#api-(.*)-)(.*)\)/, (m, g1, g2, g3, g4, g5, g6) => `${g1}(${g2 ? `/doc/rest/api${g2}/redoc-index.html` : ''}#operation/${g6.charAt(0).toLowerCase() + g6.slice(1)})`) : value;
	} catch (err) {
		return value;
	}
}
